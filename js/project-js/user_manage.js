...
$(document).ready(function() {
   ...
   //点击用户 查阅用户资料
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        for(var i=1;i<=data.pageNum;i++){
                            $(".user_page_Technican").append(" <li><a id='"+data.pageNum+"'href='#'>"+i+"</a></li>");
                        }
    ...
    //分页技师
    $(function () {
        $(document).on("click",".user_page_1",function () {
            var edit_button_id=$(this).attr("id");
            $.ajax({
                url:"http://xihashao.tunnel.2bdata.com/JGX/repairUnitAndTechnician_queryTechnican.json",
                type:'POST',
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        $(".user_page_Technican").empty();
                        for(var i=1;i<=data.pageNum;i++){
                            $(".user_page_Technican").append(" <li><a id='"+data.pageNum+"'>"+i+"</a></li>");
                        }
...
//楼盘下拉
    ...
    //点击弹出技师编辑事件
    $(function () {
        $(document).on("click",".edit_td button",function () {
            var edit_button_id=$(this).attr("id");
            $.ajax({
                url:"http://xihashao.tunnel.2bdata.com/JGX/repairUnitAndTechnician_queryTechnican.json",
                type:'GET',
...
$(function () {
    ...
    //点击弹出派单员编辑事件
    $(function () {
        $(document).on("click",".edit_td_1 button",function () {
            var edit_button_id=$(this).attr("id");
            $.ajax({
                url:"http://xihashao.tunnel.2bdata.com/JGX/repairUnitAndTechnician_queryDevelopUser.json",
                type:'GET',
